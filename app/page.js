...
export default function Home() {
  ...
  return (
    <main className="bg-white container mx-auto p-8 my-10 rounded-xl shadow-lg max-w-3xl">
      <h1 className="font-bold text-2xl text-center text-gray-1000 mb-8">
      ...
      <div className="flex flex-col items-center mb-8">
        ...
        <button
          ...
          className="w-full md:w-1/3 bg-red-500 hover:bg-red-600 text-white font-semibold px-4 py-2 rounded-lg transition duration-300"
        >
          Add Todo
        </button>
      </div>
      ...
      {todos.length > 0 ? (
        <ul className="list-none space-y-4">
          {todos.map((todo, index) => (
            <li
              key={index}
              className="flex justify-between items-center bg-red-100 p-4 rounded-lg shadow-sm"
            >
        ...
        <p className="text-center text-gray-500">No todos yet! Click the Above Button to Add one above.</p>
      )}
